...
w_socks = []    #송신 소켓 목록
msgQueues = {}  # 메세지 큐의 목록 {소켓: 메세지큐}

while r_socks:
    readEvent, writeEvent, errorEvent = select.select(r_socks, w_socks, r_socks)
    ...
    for s in readEvent: # 읽기 가능 소켓 조사
        if s is s_sock: # 서버 소켓인가?
            ...
            msgQueues[c_sock] = queue.Queue()   # 큐생성
        else:   # 클라이언트 소켓인가?
            data = s.recv(1024) 
            if data:    # 수신데이터가 있으면
                msgQueues[s].put(data)  #수신 메세지를 큐에 저장
                ...
                s.close()
                del msgQueues[s]    # 메세지 큐 제거
    
    for s in writeEvent:    # 쓰기 가능 소켓 조사
        try:
            next_msg = msgQueues[s].get_nowait()    # 메세지 큐에서 메세지 인출
        except queue.Empty: #큐가 비었으면
            w_socks.remove(s)   # 송신 소켓 목록에서 제거
        else:
            s.send(next_msg)    #인출된 메세지 전송

    for s in errorEvent:    # 오류발생 소켓 조사
        r_socks.remove(s)   # 수신 소켓 목록에서 제거
        if s in w_socks:
            w_socks.remove(s)   #송신 소켓 목록에서 제거
        s.close()
        del msgQueues[s]    #해당 소켓의 메세지 큐 제거
